...
export const SymmetricIconCanvas: React.FC<CanvasProps> = ({ params, isRunning, clearTrigger }) => {
  ...
  const draw = useCallback(() => {
    ...
    for (let k = 0; k < speed; k++) {
      ...
      // JS modulo of negative numbers is negative, Math.floor handles standard mathematical wrapping correctly
      // e.g. -0.1 - floor(-0.1) = -0.1 - (-1) = 0.9. Correct.
      ...
      for (let i = 0; i < nperiod; i++) {
        for (let j = 0; j < nperiod; j++) {
            ...
            // Let's keep it standard math coordinates where 0,0 is bottom left for consistency with math usually,
            // or strictly follow QBASIC (0,0 top left usually but formula has 'npixely - ...').
